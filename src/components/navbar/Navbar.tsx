...
export const Navbar = () => {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-white max-h-[3.4rem] width-full px-6 rounded-t-xl md:hidden">
            <ul className="flex relative w-full justify-between" ref={menuRef}>
                <span className={`bg-[#646cff] h-12 w-12 absolute -top-6 rounded-full duration-700`} style={{ transform: `translateX(${translateX}px)` }}></span>
                {menus.map((menu, index) => (
                    <li key={index} className="w-12">
                        <Link to={menu.path} className="flex flex-col text-center pt-2" onClick={() => setActive(index)}>
                            <span className={`z-100 text-xl cursor-pointer duration-500 ${index === active && "-mt-6 text-white"}`}>{menu.icon}</span>
                            <span className={`text-xs mt-0.5 ${active === index ? "translate-y-4 duration-700 opacity-100" : "translate-y-10 opacity-0"}`}>{menu.name}</span>
                        </Link>
                    </li>
                ))}
            </ul>
